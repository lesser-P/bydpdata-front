  ...
  <div class="app-container">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button size="mini" type="primary" @click="updateBrandSale"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
<script>
import branddate from "@/api/update/branddate";
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  methods: {
    ...
    async updateBrandSale() {
      await branddate.BrandDate(this.form).then((resp) => {
        if (resp.code === 200) {
          this.$message({
            message: "修改成功",
            type: "success",
    ...
    async fetchData() {
      ...
      await branddate.getBrandDateList(this.pagination).then((resp) => {
        if (resp.code === 200) {
          this.branddataList = resp.data.records;
