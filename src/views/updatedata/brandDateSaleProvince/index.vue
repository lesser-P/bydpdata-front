<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column fixed label="省份ID" align="center">
        <template slot-scope="scope">
...
<script>
import brandprovince from "@/api/update/branddateprovince.js";
export default {
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  methods: {
    ...
    async updateBrandSaleProvince() {
      await brandprovince.BrandDateProvince(this.form).then((resp) => {
        if (resp.code === 200) {
          this.$message({
            message: "修改成功",
            type: "success",
    ...
    async fetchData() {
      ...
      await brandprovince
        .getBrandDateProvinceList(this.pagination)
        .then((resp) => {
          if (resp.code === 200) {
            this.brandprovinceList = resp.data.records;
